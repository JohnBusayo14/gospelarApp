...
❌ Cannot connect to PostgreSQL.
  ...
  try {
    ...
    // Per-category topic columns on lesson_translations (added later than the
    // base table in server.js, so older deployments need them backfilled).
    await run(`ALTER TABLE lesson_translations ADD COLUMN IF NOT EXISTS topic_for_adults       TEXT`);
    await run(`ALTER TABLE lesson_translations ADD COLUMN IF NOT EXISTS topic_for_youth        TEXT`);
    await run(`ALTER TABLE lesson_translations ADD COLUMN IF NOT EXISTS topic_for_intermediate TEXT`);
    await run(`ALTER TABLE lesson_translations ADD COLUMN IF NOT EXISTS topic_for_children     TEXT`);
